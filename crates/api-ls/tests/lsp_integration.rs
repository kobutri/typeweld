...
#[test]
fn api_ls_binary_exercises_editor_facing_cross_language_behavior() {
    ...
    .join("");
    let messages = run_api_ls(&fixture.root, &transcript);
...
"#;
    ...
    let mut graph: Value =
        serde_json::from_str(&api_gen_effect_v4::render_symbol_graph(&contract, &package))
...
fn write_rust_sources(root: &Path) -> RustSources {
    let users_rs = r#"use api_macros::api;
...
"#;
    let errors_rs = r"use api_macros::api_error;
...
fn write_usage_index_after_create_user_usage_removed(root: &Path, contract_hash: &str) {
    write_file(
        ...
        &serde_json::to_string_pretty(&json!({
            ...
            "package_name": api_test_fixtures::BASIC_PACKAGE_NAME,
            "endpoints": [
...
fn run_api_ls(root: &Path, input: &str) -> Vec<Value> {
    let mut child = Command::new(env!("CARGO_BIN_EXE_api-ls"))
        .current_dir(root)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn api-ls binary");
    child
        .stdin
        .as_mut()
        .expect("api-ls stdin")
        .write_all(input.as_bytes())
        .expect("write api-ls input");
    drop(child.stdin.take());

    let output = child.wait_with_output().expect("wait for api-ls");
    assert!(
...
fn assert_initialize_succeeded(messages: &[Value]) {
    ...
    assert_eq!(response["result"]["serverInfo"]["name"], json!("api-ls"));
}
                ...
                && message["params"]["diagnostics"]
                    ...
                    .is_some_and(|items| {
                        items.iter().any(|diagnostic| {
                            diagnostic["code"] == "api::unused_endpoint"
