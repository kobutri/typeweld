...
use typeweld_build::{check_usage_lints, UsageLintConfig, UsageLintLevel};
use typeweld_cli::{
...
use typeweld_gen_effect_v4::{render_generated_package, render_symbol_graph, GeneratedPackage};
use typeweld_ir::{ApiContract, Field, SourceRange, SourceSpan, TypeShape};
...
fn run_with_args(args: impl IntoIterator<Item = String>) -> Result<(), String> {
    let cli = ApiCli::try_parse_from(std::iter::once("typeweld".to_owned()).chain(args))
...
fn collect_command(options: CollectOptions) -> Result<(), String> {
    if options.empty {
        let out = options
            .out
            .clone()
            .ok_or_else(|| "typeweld collect --empty requires --out <path>".to_owned())?;
        let package_name = options.package_name.clone().ok_or_else(|| {
            "typeweld collect --empty requires --package-name <ts-package-name>".to_owned()
        })?;
    ...
    let out = options
        .out
        .clone()
        .ok_or_else(|| "typeweld collect requires --out <path>".to_owned())?;
...
fn gen_command(options: GenOptions) -> Result<(), String> {
    ...
    let checked_contracts = check_collect_contracts(&check_options, "typeweld gen")?;
...
fn run_full_check(options: &CheckOptions) -> Result<(), String> {
    let checked_contracts = check_collect_contracts(options, "typeweld check")?;
    ...
    println!(
        "typeweld check passed for {} contract(s)",
...
fn check_run_usage_lints(
    ...
    if !usage_index_available {
        return Err(
            "typeweld check cannot run unused endpoint lints without a usage index".to_owned(),
        );
    }
    ...
    {
        return Err(format!(
            "typeweld check failed because {} unused endpoint diagnostic(s) were denied",
...
fn check_usages_command(options: CheckUsagesOptions) -> Result<(), String> {
    ...
    if let Some(symbol_graph) = symbol_graph {
        if symbol_graph.is_file() {
            ...
            return Err(format!(
                "typeweld check-usages --symbol-graph path does not exist: {}",
...
fn doctor_command(options: DoctorOptions) -> Result<(), String> {
    ...
    let workspace = discover_workspace(&manifest_path).map_err(|error| {
        format!(
            "typeweld doctor could not inspect `{}`: {error}\nhelp: pass --manifest-path <Cargo.toml>",
    ...
    println!(
        "setup: run `typeweld collect`, `typeweld gen`, and `typeweld check-usages` before enabling deny lints"
    );
    Ok(())
}
...
fn new_project_command(options: NewProjectOptions) -> Result<(), String> {
    ...
    println!(
        "next: cd {} && npm install && npm run typeweld:check && npm run typecheck",
...
fn resolve_new_project(options: NewProjectOptions) -> Result<NewProject, String> {
    ...
    if name.is_empty() {
        return Err(
            "typeweld new project name must contain at least one ASCII letter or digit".to_owned(),
        );
    }
    ...
    if !is_rust_identifier(&api_module) {
        return Err("typeweld new module name must be a valid Rust identifier".to_owned());
    }
    ...
    bind_addr
        ...
        .map_err(|error| format!("typeweld new --bind requires a socket address: {error}"))?;
...
fn write_new_project(project: &NewProject) -> Result<(), String> {
    if project.root_dir.exists() && !project.force && directory_has_entries(&project.root_dir)? {
        return Err(format!(
            "typeweld new target directory `{}` is not empty; pass --force to write scaffold files there",
            project.root_dir.display()
        ));
    }
    ...
    write_project_file(
        project,
        ".typeweld.json",
        render_new_typeweld_ls_json(project),
    )?;
...
fn render_new_root_package_json(project: &NewProject) -> String {
  ...
  "scripts": {{
    "typeweld:check": {api_check},
    "typeweld:gen": {api_gen},
    "app": {app},
...
fn render_new_app_package_json(project: &NewProject) -> String {
  ...
  "dependencies": {{
    "@typeweld/effect-runtime": {effect_runtime},
...
"#,
        ...
        effect_version = json_string(typeweld_gen_effect_v4::EFFECT_VERSION),
    )
}
...
Minimal Rust + TypeScript API starter generated by `typeweld new`.
...
`npm run typeweld:check` writes it under `target/api-contract/effect-v4/packages`.

## LSP

This project includes `.typeweld.json`. Point your editor at:

```sh
npm exec -- typeweld-ls
```

Use `typeweld-ls` as the only language server for Rust and TypeScript files in this
workspace.
"#,
        name = project.name,
...
#[typeweld_macros::api(method = "GET", path = "/{route_prefix}/{{name}}")]
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {{
    ...
    {{
        Command::Contract => println!(
            "{{}}",
            typeweld_cli::contract_to_json(&{crate_name}::contract())?
        ),
...
fn render_new_app_tsconfig(project: &NewProject) -> String {
    let package_dir =
        typeweld_gen_effect_v4::generated_package_dir(Path::new("../target"), &project.ts_package);
...
fn render_new_app_client(project: &NewProject) -> String {
    format!(
        r#"import {{ Effect }} from "@typeweld/effect-runtime/compat"
...
fn validate_cargo_package_name(name: &str) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "typeweld new Rust package `{name}` must use ASCII letters, digits, '-' or '_'"
        ));
    }
    Ok(())
}
...
fn validate_ts_package_name(name: &str) -> Result<(), String> {
    if name.trim().is_empty() || name.contains(char::is_whitespace) {
        return Err(
            "typeweld new TypeScript package name must be non-empty and contain no whitespace"
                .to_owned(),
        );
    }
    Ok(())
...
fn load_cargo_metadata(manifest_path: &Path) -> Result<cargo_metadata::Metadata, String> {
    cargo_metadata::MetadataCommand::new()
        ...
        .map_err(|error| {
            format!(
                "typeweld collect could not inspect `{}`: {error}",
                manifest_path.display()
            )
        })
}
...
fn collect_workspace_contracts(options: &CollectOptions) -> Result<PathBuf, String> {
    ...
    if packages.is_empty() {
        return Err(
            "typeweld collect --workspace found no packages with [package.metadata.rust_ts]"
                .to_owned(),
        );
    }
...
fn resolve_collect_package<'a>(
    ...
    if let Some(package_name) = package_name {
        return metadata
            ...
            .ok_or_else(|| {
                format!("typeweld collect could not find workspace package `{package_name}`")
            });
    }
    ...
    match api_packages.as_slice() {
        [package] => Ok(*package),
        [] => Err("typeweld collect requires --package <cargo-package> because no workspace package declares [package.metadata.rust_ts]".to_owned()),
        packages => Err(format!(
            "typeweld collect found multiple API-enabled packages ({}) ; pass --package <cargo-package>",
            packages
                .iter()
...
fn resolve_collect_target(
    ...
    let ts_package_name = options
        ...
        .ok_or_else(|| {
            format!(
                "typeweld collect package `{}` requires --package-name <ts-package-name> or package.metadata.rust_ts.ts_package",
                package.name
            )
        })?;
    let api_root = options
        ...
        .ok_or_else(|| {
            format!(
                "typeweld collect package `{}` requires --api-root <path::to::api> or package.metadata.rust_ts.api_root",
                package.name
            )
        })?;
...
fn package_lib_target(
    ...
    package
        ...
        .ok_or_else(|| {
            format!(
                "typeweld collect package `{}` must expose a library target so the temporary collector can depend on it",
                package.name
            )
        })
}
...
fn normalize_api_root(api_root: &str, lib_crate_name: &str) -> Result<Vec<String>, String> {
    let mut parts = api_root
        ...
        .collect::<Vec<_>>();
    if parts.is_empty() {
        return Err("typeweld collect --api-root must name a Rust function path".to_owned());
    }
    ...
    if parts.iter().any(|part| !is_rust_identifier(part)) {
        return Err(format!(
            "typeweld collect --api-root `{api_root}` contains a non-identifier path segment"
        ));
    }
    Ok(parts)
}
...
fn write_temp_collector_crate(
    ...
    let typeweld_cli_path = tool_package_manifest_dir(metadata, "typeweld-cli")?;
    let typeweld_core_path = tool_package_manifest_dir(metadata, "typeweld-core")?;
...
"#,
        ...
        typeweld_cli_path = toml_string(&typeweld_cli_path.display().to_string()),
        typeweld_core_path = toml_string(&typeweld_core_path.display().to_string()),
    ...
    let main_rs = format!(
        r#"fn main() {{
    let contract = typeweld_cli::collect_contract(typeweld_cli::CollectorInput::from_root(
        {ts_package_name}.to_owned(),
        {root_call},
    ));
    println!(
        "{{}}",
        typeweld_cli::contract_to_json(&contract).expect("collected contract should serialize")
    );
}}
"#,
...
fn run_temp_collector(collector_dir: &Path) -> Result<ApiContract, String> {
    ...
    if !output.status.success() {
        return Err(format!(
            "typeweld collect failed while compiling or running the temporary collector `{}`; Cargo diagnostics were printed above",
            collector_dir.display()
        ));
    }
...
fn tool_package_manifest_dir(
    ...
    match package_name {
        "typeweld-cli" => Ok(collector_dir),
        "typeweld-core" => collector_dir
            .parent()
            .map(|crates_dir| crates_dir.join("typeweld-core"))
            .filter(|path| path.join("Cargo.toml").is_file())
            .ok_or_else(|| {
                "typeweld collect could not locate typeweld-core next to the running typeweld-cli source"
                    .to_owned()
            }),
        _ => Err(format!(
            "typeweld collect could not locate tool package `{package_name}`"
        )),
    }
}
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("typeweld-cli is under crates/")
        .to_path_buf()
}
...
fn run_watch_generation(options: &WatchOptions) -> Result<(), String> {
    let check_options = options.check_options();
    let checked_contracts = check_collect_contracts(&check_options, "typeweld watch")?;
    let packages = check_generate_contracts(&checked_contracts, &check_options.target_dir)?;
    println!(
...
fn write_workspace_tsconfig_paths(
    ...
    for package in &graph.packages {
        let package_dir =
            typeweld_gen_effect_v4::generated_package_dir(target_dir, &package.ts_package);
        let package_dir = normalize_path(&package_dir);
        entries.push(format!(
            "      {package_name}: [\n        {index_path}\n      ]",
...
fn usage_location_json(file: &str, source: &typeweld_ir::SourceRange) -> serde_json::Value {
...
fn source_range_json(source: &typeweld_ir::SourceRange) -> serde_json::Value {
    serde_json::json!({
        "start": {
...
mod tests {
    ...
    fn new_project_generates_minimal_workspace() {
        ...
        assert!(root.join(".typeweld.json").is_file());
        ...
        assert!(!root_manifest.contains("\"typeweld:collect\""));
        ...
        assert!(server_lib.contains("#[typeweld_macros::api_router]"));
    ...
    fn new_project_registry_source_uses_published_packages() {
        ...
        let root_manifest =
            fs::read_to_string(root.join("package.json")).expect("read root manifest");
        assert!(root_manifest.contains("\"typeweld\": \"0.0.1\""));
        assert!(root_manifest.contains("\"@typeweld/language-server\": \"0.0.1\""));
        assert!(!root_manifest.contains("cargo run --manifest-path"));
        assert!(!root_manifest.contains("file:"));

        let app_manifest =
            fs::read_to_string(root.join("app/package.json")).expect("read app manifest");
        assert!(app_manifest.contains("\"@typeweld/effect-runtime\": \"0.0.1\""));
        assert!(!app_manifest.contains("file:"));

        let server_manifest =
            fs::read_to_string(root.join("server/Cargo.toml")).expect("read server manifest");
        assert!(server_manifest.contains("typeweld-axum = \"0.0.1\""));
        assert!(server_manifest
            .contains("typeweld-cli = { version = \"0.0.1\", default-features = false }"));
        assert!(server_manifest.contains("typeweld-core = \"0.0.1\""));
        assert!(server_manifest.contains("typeweld-macros = \"0.0.1\""));
    ...
    fn new_project_github_source_uses_release_assets_and_git_deps() {
        ...
        assert!(root_manifest.contains(
            "https://github.com/kobutri/typeweld/releases/download/v0.0.1/typeweld-0.0.1.tgz"
        ));
        assert!(root_manifest.contains(
            "https://github.com/kobutri/typeweld/releases/download/v0.0.1/typeweld-language-server-0.0.1.tgz"
        ));

        let app_manifest =
            fs::read_to_string(root.join("app/package.json")).expect("read app manifest");
        assert!(app_manifest.contains(
            "https://github.com/kobutri/typeweld/releases/download/v0.0.1/typeweld-effect-runtime-0.0.1.tgz"
        ));

        let server_manifest =
            fs::read_to_string(root.join("server/Cargo.toml")).expect("read server manifest");
        assert!(server_manifest.contains(
            "typeweld-axum = { git = \"https://github.com/kobutri/typeweld\", tag = \"v0.0.1\" }"
        ));
        assert!(server_manifest.contains(
            "typeweld-cli = { git = \"https://github.com/kobutri/typeweld\", tag = \"v0.0.1\", default-features = false }"
        ));
    }
    ...
    fn gen_and_check_hidden_package() {
        ...
        fs::write(
            &contract_path,
            serde_json::to_string_pretty(&typeweld_test_fixtures::basic_contract())
...
serde = {{ version = "1.0", features = ["derive"] }}
"#,
                typeweld_axum_path = toml_string(
                    &repo_root()
                        .join("crates/typeweld-axum")
                        .display()
                        .to_string()
                ),
                typeweld_core_path = toml_string(
                    &repo_root()
                        .join("crates/typeweld-core")
                        .display()
                        .to_string()
                ),
                typeweld_macros_path = toml_string(
                    &repo_root()
                        .join("crates/typeweld-macros")
                        .display()
                        .to_string()
                ),
            ),
        ...
        fs::write(
            ...
            r#"use typeweld_axum::{ApiRouter, Json, Path};
use serde::Serialize;

#[derive(typeweld_macros::ApiType, Serialize)]
pub struct User {
    id: i64,
}
...
#[derive(typeweld_macros::ApiError, Serialize)]
#[serde(tag = "_tag")]
...
#[cfg(feature = "extra")]
#[typeweld_macros::api(method = "GET", path = "/audit/{id}")]
...
"#,
        ...
        assert!(typeweld_gen_effect_v4::generated_package_dir(
            &gen_target_dir,
            "@metadata/server-api"
        )
        .join("index.ts")
...
"#,
        ...
        .expect("write lib");
        let users_rs = r#"use typeweld_axum::{ApiRouter, Json, Path};
use serde::Serialize;
...
"#,
    ...
    fn test_root(name: &str) -> PathBuf {
        ...
        let path = env::temp_dir().join(format!("typeweld-cli-{name}-{}-{id}", std::process::id()));
...
"#,
        ...
        fs::write(
            ...
            format!(
                r#"use typeweld_axum::{{ApiRouter, Json}};
use serde::Serialize;
...
#[typeweld_macros::api(method = "GET", path = "{route}")]
