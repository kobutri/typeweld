...
where
    S: Clone + Send + Sync + 'static,
{
    fn into_api_module(self) -> typeweld_core::ApiModule {
...
impl<T> ApiSuccessResponse for typeweld_core::Json<T>
where
    T: Serialize,
{
    fn into_api_response(self) -> Response {
...
impl<T> ApiSuccessResponse for typeweld_core::Created<T>
where
    T: Serialize,
{
    fn into_api_response(self) -> Response {
...
impl ApiSuccessResponse for typeweld_core::NoContent {
    fn into_api_response(self) -> Response {
        StatusCode::NO_CONTENT.into_response()
    }
}

impl ApiSuccessResponse for typeweld_core::Binary<Bytes> {
    fn into_api_response(self) -> Response {
        (
            [(
                CONTENT_TYPE,
...
impl<T> ApiSuccessResponse for typeweld_core::Sse<T>
where
...
mod tests {
    ...
    use typeweld_core::{ir::HttpMethod, ApiType, ContractRegistry, Endpoint, EndpointDescriptor};
    ...
    impl ApiError for GetUserError {
        ...
        fn error_def() -> typeweld_core::ir::ErrorDef {
            typeweld_core::ir::ErrorDef {
                ...
                variants: Vec::new(),
                source: typeweld_core::ir::SourceRange::default(),
            }
        }
    }
    ...
    async fn endpoint_descriptor_mounts_runtime_handler_and_contract_node() {
        let router = ApiRouter::new("users").endpoint_descriptor(
            descriptor(HttpMethod::Get, "/users/{id}"),
            get_user,
            typeweld_core::ir::SourceRange::default(),
        );
