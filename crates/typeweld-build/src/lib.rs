...
fn write_lint_glue(
    ...
    output.push_str("// Generated by typeweld-build. Do not edit.\n");
...
fn regeneration_command(config: &UsageLintConfig) -> String {
    format!(
        "typeweld check --target-dir {} --ts-dir <ts-source-dir>",
...
mod tests {
    ...
    fn missing_usage_index_reports_actionable_instruction() {
        ...
        assert!(report.diagnostics[0]
            .message
            .contains("typeweld check --target-dir"));
    ...
    fn test_root(name: &str) -> PathBuf {
        ...
        let path =
            env::temp_dir().join(format!("typeweld-build-{name}-{}-{id}", std::process::id()));
