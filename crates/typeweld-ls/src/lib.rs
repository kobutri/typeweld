...
const OPEN_GENERATED_FILE_COMMAND: &str = "typeweld-ls.openGeneratedPackageFile";
...
/// `typeweld-ls` configuration loaded from the workspace root.
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(default, rename_all = "camelCase")]
pub struct ApiLsConfig {
    pub rust_analyzer: BackendCommand,
    pub typescript: BackendCommand,
    #[serde(alias = "apiWatch")]
    pub typeweld_watch: TypeweldWatchConfig,
...
impl Default for ApiLsConfig {
    fn default() -> Self {
        Self {
            ...
            typeweld_watch: TypeweldWatchConfig::default(),
...
/// `typeweld watch` process managed by the language-server gateway.
#[derive(Clone, Debug, Eq, PartialEq, Deserialize, Serialize)]
#[serde(default, rename_all = "camelCase")]
pub struct TypeweldWatchConfig {
    pub enabled: bool,
    pub command: String,
    pub args: Vec<String>,
}

impl Default for TypeweldWatchConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            command: String::new(),
...
/// Discovers the workspace root and loads optional `typeweld-ls` configuration.
///
...
struct AsyncLspServer {
    ...
    typeweld_watch: Option<TypeweldWatchProcess>,
...
impl AsyncLspServer {
    ...
    async fn handle_method_message(
        ...
        match method {
            "initialize" => {
                ...
                match initialize_workspace(&message.params) {
                    Ok(workspace) => {
                        ...
                        let (generated_tsconfig, typeweld_watch) =
                            match prepare_generated_package_cache(
                                &workspace,
                                self.event_tx.clone(),
                                log_file.clone(),
                            )
                        ...
                        self.typeweld_watch = typeweld_watch;
            ...
            "typeweld-ls/generatedPackageFile" => {
                if let Some(id) = message.id {
                ...
                Some(id) => {
                    if let Some(kind) = self.route_backend(method, &message.params) {
                        ...
                        self.write_error(
                            id,
                            -32_601,
                            format!("typeweld-ls has no handler for `{method}` yet"),
                        )?;
                    }
                }
                None => {
    ...
    fn handle_execute_command(&mut self, params: Value, id: Value) -> Result<(), String> {
        ...
        if let Some(kind) = self.backend_for_execute_command(command) {
            ...
            self.write_error(
                id,
                -32_601,
                format!("typeweld-ls has no handler for command `{command}`"),
            )
        }
    }
    ...
    async fn fail_backend(&mut self, kind: BackendKind, error: String) -> Result<(), String> {
        self.write_notification(
            ...
            json!({
                "type": 1,
                "message": format!("typeweld-ls {} backend unavailable: {error}", kind.display_name()),
            }),
        )?;
    ...
    async fn stop_backends(&mut self) {
        if let Some(typeweld_watch) = self.typeweld_watch.take() {
            typeweld_watch.stop().await;
        }
        if let Some(backend) = self.rust_analyzer.take() {
            backend.stop().await;
        }
    ...
    fn open_generated_package_file(&mut self, params: Value, id: Value) -> Result<(), String> {
        let Some(workspace) = &self.workspace else {
            return self.write_error(id, -32_000, "typeweld-ls is not initialized".to_owned());
        };
...
struct TypeweldWatchProcess {
    child: Child,
}

#[derive(Clone, Debug, Eq, PartialEq)]
struct ResolvedTypeweldWatchCommand {
    command: String,
    args: Vec<String>,
}

impl TypeweldWatchProcess {
    async fn start(
        workspace: &WorkspaceConfig,
        event_tx: mpsc::UnboundedSender<GatewayEvent>,
        log_file: Option<PathBuf>,
    ) -> Result<Option<Self>, String> {
        let Some(command) = resolve_typeweld_watch_command(workspace)? else {
            return Ok(None);
        };
        ...
        let mut child = Command::new(&command.command)
            ...
            .map_err(|error| {
                format!(
                    "typeweldWatch failed to start `{}`: {error}",
                    typeweld_watch_command_label(&command, &args)
                )
            })?;

        if let Some(stdout) = child.stdout.take() {
            spawn_typeweld_watch_log_task("stdout", stdout, event_tx.clone(), log_file.clone());
        }
        if let Some(stderr) = child.stderr.take() {
            spawn_typeweld_watch_log_task("stderr", stderr, event_tx, log_file);
        }
    ...
    fn try_wait(&mut self) -> Result<Option<std::process::ExitStatus>, String> {
        self.child
            .try_wait()
            .map_err(|error| format!("failed to inspect typeweldWatch process: {error}"))
    }
}
...
async fn prepare_generated_package_cache(
    ...
    if let Some(mut typeweld_watch) = typeweld_watch {
        ...
        Ok((tsconfig, Some(typeweld_watch)))
    } else {
...
async fn wait_for_generated_package_cache(
    workspace: &WorkspaceConfig,
    typeweld_watch: &mut TypeweldWatchProcess,
    ...
    loop {
        ...
        if let Some(status) = typeweld_watch.try_wait()? {
            return Err(format!(
                "typeweldWatch exited before generating `{}` with {status}",
        ...
        if started.elapsed() >= timeout {
            return Err(format!(
                "timed out waiting for typeweldWatch to generate `{}`",
                workspace.generated_cache_dir.display()
            ));
        }
...
fn resolve_typeweld_watch_command(
    workspace: &WorkspaceConfig,
) -> Result<Option<ResolvedTypeweldWatchCommand>, String> {
    if !workspace.config.typeweld_watch.enabled {
        return Ok(None);
    }
    if !workspace.config.typeweld_watch.command.trim().is_empty() {
        return Ok(Some(ResolvedTypeweldWatchCommand {
            command: workspace.config.typeweld_watch.command.clone(),
            args: workspace.config.typeweld_watch.args.clone(),
        }));
    }

    Ok(
        sibling_typeweld_binary().map(|command| ResolvedTypeweldWatchCommand {
            command,
            args: workspace.config.typeweld_watch.args.clone(),
        }),
    )
...
fn sibling_typeweld_binary() -> Option<String> {
    let executable = env::current_exe().ok()?;
    let sibling = executable.parent()?.join(platform_typeweld_binary_name());
    sibling
        .is_file()
        .then(|| sibling.to_string_lossy().into_owned())
}

fn platform_typeweld_binary_name() -> &'static str {
    if cfg!(windows) {
        "typeweld.exe"
    } else {
        "typeweld"
...
fn typeweld_watch_command_label(command: &ResolvedTypeweldWatchCommand, args: &[String]) -> String {
...
fn typeweld_watch_start_diagnostic(workspace: &WorkspaceConfig, error: &str) -> String {
    let command = workspace_typeweld_watch_command_label(workspace);
    [
        "typeweld-ls could not start typeweldWatch to refresh generated TypeScript packages.".to_owned(),
        ...
        "Install or build the project CLI, set `typeweldWatch.command` in `.typeweld.json` to the executable path, or run `typeweld gen` before starting the editor.".to_owned(),
    ]
    .join("\n")
}
...
fn generated_cache_diagnostic(workspace: &WorkspaceConfig, error: &str) -> String {
    ...
    if workspace.config.typeweld_watch.enabled {
        lines.push(
            "typeweldWatch is enabled, but typeweld-ls did not find a sibling `typeweld` binary and no `typeweldWatch.command` is configured.".to_owned(),
        );
        lines.push(
            "Set `typeweldWatch.command` in `.typeweld.json` to your project CLI, or run `typeweld gen` once before starting the editor.".to_owned(),
        );
    } else {
        lines.push(
            "typeweldWatch is disabled, so typeweld-ls cannot generate the cache automatically."
                .to_owned(),
        );
        lines.push(
            "Enable `typeweldWatch`, or run `typeweld gen` before starting the editor.".to_owned(),
        );
    }
    lines.join("\n")
}

fn workspace_typeweld_watch_command_label(workspace: &WorkspaceConfig) -> String {
    if !workspace.config.typeweld_watch.command.trim().is_empty() {
        return typeweld_watch_command_label(
            &ResolvedTypeweldWatchCommand {
                command: workspace.config.typeweld_watch.command.clone(),
                args: workspace.config.typeweld_watch.args.clone(),
            },
            &workspace.config.typeweld_watch.args,
        );
    }

    std::iter::once(platform_typeweld_binary_name())
        .chain(
            workspace
                .config
                .typeweld_watch
...
fn spawn_typeweld_watch_log_task<R>(
    ...
    tokio::spawn(async move {
        ...
        loop {
            match lines.next_line().await {
                ...
                Err(error) => {
                    let _ = event_tx.send(GatewayEvent::BackendLog(format!(
                        "typeweldWatch {stream_name} read failed: {error}"
                    )));
                    break;
                }
            }
...
async fn log_typeweld_watch_message(log_file: Option<PathBuf>, message: &str) {
    if let Some(path) = log_file {
...
fn backend_start_diagnostic(
    ...
    let mut lines = vec![
        format!("typeweld-ls could not start the {label} backend."),
    ...
    if let Some(path) = &workspace.config_path {
        ...
        lines.push("Config file: none; typeweld-ls is using its defaults.".to_owned());
    }
    ...
    match error {
        ...
        BackendStartFailure::MissingStdio(stream) => {
            lines.push(format!(
                "The process started, but typeweld-ls could not attach to {stream}."
            ));
        }
        BackendStartFailure::InitializeWrite(write_error) => {
            lines.push(format!(
                "The process started, but typeweld-ls could not send the LSP initialize request: {write_error}."
            ));
        }
...
fn backend_install_help(kind: BackendKind) -> Vec<String> {
    match kind {
        BackendKind::Rust => vec![
            ...
            "If rust-analyzer lives somewhere else, set `rustAnalyzer.command` in `.typeweld.json` to its absolute path.".to_owned(),
        ],
        BackendKind::TypeScript => vec![
            ...
            "For a project-local install, set `typescript.command` in `.typeweld.json` to `./node_modules/.bin/typescript-language-server` and keep `typescript.args` as `[\"--stdio\"]`.".to_owned(),
        ],
    }
}
...
fn typescript_initialize_params(
    ...
    params["initializationOptions"]["hostInfo"] = json!("typeweld-ls");
...
fn discover_generated_packages(
    ...
    if !packages_dir.is_dir() {
        return Err(format!(
            "generated package cache `{}` does not exist; run `typeweld gen --contract <path> --target-dir <target>` before starting typeweld-ls",
    ...
    for entry in entries {
        ...
        if !index_path.is_file() {
            return Err(format!(
                "generated package `{}` is missing index.ts; rerun `typeweld gen`",
                package_dir.display()
            ));
        }
    ...
    if packages.is_empty() {
        return Err(format!(
            "generated package cache `{}` is empty; run `typeweld gen --contract <path> --target-dir <target>` before starting typeweld-ls",
            packages_dir.display()
        ));
    }
...
fn route_placeholder_range(
    ...
    let attr_start = route_macro_attribute_start(prefix)?;
...
fn route_macro_attribute_start(prefix: &str) -> Option<usize> {
    prefix.rfind("#[api").or_else(|| {
        let macro_path = prefix.rfind("::api")?;
        prefix.get(..macro_path)?.rfind("#[")
    })
}
...
mod tests {
  ...
  "rustAnalyzer": { "command": "ra-test", "args": ["--log"] }
        ...
        assert!(config.config.typeweld_watch.enabled);
        assert_eq!(config.config.typeweld_watch.args, vec!["watch".to_owned()]);
    }
    ...
    fn initialize_and_shutdown_use_lsp_framing() {
        ...
        assert!(output.contains("\"serverInfo\":{\"name\":\"typeweld-ls\""));
    ...
    fn invalid_config_reports_clear_initialize_error() {
        ...
        fs::create_dir_all(&root).expect("create root");
        fs::write(root.join("Cargo.toml"), "[workspace]\n").expect("write manifest");
        fs::write(root.join(".typeweld.json"), "{ nope").expect("write config");
        ...
        assert!(output.contains(".typeweld.json"));
    }
    ...
    fn missing_rust_backend_executable_suggests_install() {
        ...
        fs::write(
            root.join(".typeweld.json"),
            json!({
                "rustAnalyzer": { "command": "definitely-missing-rust-analyzer-for-typeweld-ls" },
                "typescript": { "command": "python3", "args": ["-c", MOCK_BACKEND] }
            })
            .to_string(),
        )
        ...
        assert!(output.contains("definitely-missing-rust-analyzer-for-typeweld-ls"));
    ...
    fn missing_typescript_backend_executable_suggests_install() {
        ...
        fs::write(
            ...
            json!({
                ...
                "typescript": { "command": "definitely-missing-typescript-language-server-for-typeweld-ls" }
            })
            .to_string(),
        )
        .expect("write config");
        ...
        assert!(output.contains("definitely-missing-typescript-language-server-for-typeweld-ls"));
        assert!(output.contains("was not found on PATH"));
        assert!(output.contains("npm install -D typescript typescript-language-server"));
    ...
    fn captures_backend_stderr_to_configured_log_file() {
        ...
        run(input.as_bytes(), &mut output).expect("run server");
        let log = fs::read_to_string(root.join("target/typeweld-ls.log")).expect("read log");
    ...
    fn missing_generated_package_cache_fails_before_typescript_initialize() {
        ...
        assert!(output.contains("typeweld gen"));
    }
    ...
    fn rust_references_use_generated_typescript_semantics_and_hide_generated_files() {
        ...
        let package =
            typeweld_gen_effect_v4::render_generated_package(&contract, &root.join("target"));
        write_generated_package_files(&package);
        let symbol_graph = typeweld_gen_effect_v4::render_symbol_graph(&contract, &package);
                ...
                framed(&json!({
                    ...
                    "params": {
                        "textDocument": {
                            ...
                            "languageId": "rust",
                            "version": 1,
                            "text": "fn routes() {\n    typeweld_axum::ApiRouter::new(\"server\")\n        .endpoint(get_user);\n}\n"
                        }
                    }
                })),
                framed(&json!({
    ...
    fn route_param_rename_updates_route_path_rust_arg_and_ts_usage() {
        ...
        let rust_source = concat!(
            "#[typeweld_macros::api(method = \"GET\", path = \"/users/{user_id}\")]\n",
    ...
    fn publishes_unused_endpoint_diagnostics_from_usage_index() {
        ...
        assert!(output.contains("\"method\":\"textDocument/publishDiagnostics\""));
        assert!(output.contains("typeweld::unused_endpoint"));
    ...
    fn unused_endpoint_diagnostics_support_deny_and_off_modes() {
        ...
        assert!(!off_output.contains("typeweld::unused_endpoint"));
    ...
    fn test_root(name: &str) -> PathBuf {
        ...
        let path = env::temp_dir().join(format!("typeweld-ls-{name}-{}-{id}", std::process::id()));
    ...
    fn write_backend_config(
        ...
        fs::write(root.join(".typeweld.json"), config.to_string()).expect("write config");
    }
