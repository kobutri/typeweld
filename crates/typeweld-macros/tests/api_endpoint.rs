use axum::{body::to_bytes, http::StatusCode};
use typeweld_core::{
    ir::{HttpMethod, RequestBodyTransport, ResponseShape, Transport},
    Binary, Created, Json, Path, Query, Sse,
};

fn endpoint(descriptor: typeweld_core::EndpointDescriptor) -> typeweld_core::Endpoint {
    descriptor.into_endpoint()
}

#[derive(typeweld_macros::ApiType, serde::Deserialize, serde::Serialize)]
#[allow(dead_code)]
struct User {
    id: i64,
}

#[derive(typeweld_macros::ApiError, serde::Serialize)]
#[serde(tag = "_tag")]
#[allow(dead_code)]
enum GetUserError {
    #[api_error(status = 404)]
    NotFound,
}

#[derive(typeweld_macros::ApiError, serde::Serialize)]
#[serde(tag = "_tag")]
#[allow(dead_code)]
enum SearchUsersError {
    #[api_error(status = 400)]
    BadFilter,
}

#[typeweld_macros::api(method = "GET", path = "/users/{id}", allow_unused)]
#[allow(dead_code)]
async fn get_user(id: Path<i64>, filter: Query<String>) -> Result<Json<User>, GetUserError> {
    let _ = (id, filter);
    unimplemented!()
}

#[typeweld_macros::api(method = "POST", path = "/users")]
#[allow(dead_code)]
async fn create_user() -> Result<Created<User>, GetUserError> {
    unimplemented!()
}

#[typeweld_macros::api(method = "SSE", path = "/users/events")]
#[allow(dead_code)]
fn user_events() -> Result<Sse<User>, GetUserError> {
    unimplemented!()
}

#[typeweld_macros::api(method = "GET", path = "/users/search")]
#[allow(dead_code)]
async fn search_users() -> Result<Json<User>, SearchUsersError> {
    unimplemented!()
}

#[typeweld_macros::api(method = "POST", path = "/axum-users/{id}")]
#[allow(dead_code)]
async fn axum_wrapped_create_user(
    id: typeweld_axum::Path<i64>,
    body: typeweld_axum::Body<User>,
) -> Result<typeweld_axum::Created<User>, GetUserError> {
    let _ = (id, body);
    std::future::ready(()).await;
    unimplemented!()
}

type UserEventStream =
    futures_util::stream::Iter<std::array::IntoIter<Result<User, GetUserError>, 1>>;

#[typeweld_macros::api(method = "SSE", path = "/axum-users/events")]
#[allow(dead_code)]
fn axum_wrapped_user_events() -> Result<typeweld_axum::Sse<User, UserEventStream>, GetUserError> {
    unimplemented!()
}

#[typeweld_macros::api(method = "GET", path = "/axum-users/plain")]
#[allow(dead_code)]
async fn axum_plain_user() -> typeweld_axum::Json<User> {
    typeweld_axum::Json(User { id: 7 })
}

#[typeweld_macros::api(method = "GET", path = "/axum-users/result")]
#[allow(dead_code)]
async fn axum_result_user() -> Result<typeweld_axum::Json<User>, GetUserError> {
    Ok(typeweld_axum::Json(User { id: 9 }))
}

#[typeweld_macros::api(method = "GET", path = "/axum-users/missing")]
#[allow(dead_code)]
async fn axum_missing_user() -> Result<typeweld_axum::Json<User>, GetUserError> {
    Err(GetUserError::NotFound)
}

#[typeweld_macros::api(method = "SSE", path = "/axum-users/plain-events")]
#[allow(dead_code)]
fn axum_plain_events() -> typeweld_axum::Sse<User, UserEventStream> {
    typeweld_axum::Sse::new(futures_util::stream::iter([Ok(User { id: 11 })]))
}

#[typeweld_macros::api(method = "SSE", path = "/axum-users/result-events")]
#[allow(dead_code)]
fn axum_result_events() -> Result<typeweld_axum::Sse<User, UserEventStream>, GetUserError> {
    Ok(typeweld_axum::Sse::new(futures_util::stream::iter([Ok(
        User { id: 13 },
    )])))
}

#[typeweld_macros::api(method = "GET", path = "/files/{id}")]
#[allow(dead_code)]
async fn download_file(id: Path<i64>) -> Result<Binary<bytes::Bytes>, GetUserError> {
    let _ = id;
    std::future::ready(()).await;
    unimplemented!()
}

#[typeweld_macros::api(method = "GET", path = "/axum-files/{id}")]
#[allow(dead_code)]
async fn axum_download_file(id: typeweld_axum::Path<i64>) -> typeweld_axum::Binary<bytes::Bytes> {
    let _ = id;
    typeweld_axum::Binary(bytes::Bytes::from_static(b"file"))
}

#[typeweld_macros::api(method = "POST", path = "/axum-files/{id}")]
#[allow(dead_code)]
async fn axum_upload_file(
    id: typeweld_axum::Path<i64>,
    body: typeweld_axum::Binary<bytes::Bytes>,
) -> typeweld_axum::Json<User> {
    let _ = (id, body);
    typeweld_axum::Json(User { id: 17 })
}

#[typeweld_macros::api_router]
fn declarative_routes() -> typeweld_axum::ApiRouter {
    typeweld_axum::ApiRouter::new("users")
        .endpoint(axum_plain_user)
        .endpoint(axum_result_user)
        .route("/healthz", axum::routing::get(|| async { "ok" }))
}

#[typeweld_macros::api(method = "POST", path = "/files/{id}")]
#[allow(dead_code)]
async fn upload_file(
    id: Path<i64>,
    body: Binary<bytes::Bytes>,
) -> Result<Json<User>, GetUserError> {
    let _ = (id, body);
    std::future::ready(()).await;
    unimplemented!()
}

#[test]
fn api_endpoint_macro_emits_metadata() {
    let endpoint = endpoint(__api_endpoint_descriptor_get_user());

    assert_eq!(endpoint.method, HttpMethod::Get);
    assert_eq!(endpoint.route.0, "/users/{id}");
    assert_eq!(endpoint.rust_name, "get_user");
    assert_eq!(endpoint.ts_path, ["users", "getUser"]);
    assert!(endpoint.allow_unused);
    assert_eq!(endpoint.request.path_params[0].rust_name, "id");
    assert_eq!(endpoint.request.query_params[0].rust_name, "filter");
    assert_eq!(endpoint.errors[0].name, "GetUserError");

    let ResponseShape::Json(response) = endpoint.response else {
        panic!("expected json response");
    };
    assert_eq!(response.name, "User");
}

#[test]
fn api_endpoint_macro_emits_sse_metadata() {
    let endpoint = endpoint(__api_endpoint_descriptor_user_events());

    assert_eq!(endpoint.method, HttpMethod::Get);
    assert_eq!(endpoint.transport, Transport::ServerSentEvents);
    assert_eq!(endpoint.route.0, "/users/events");
    assert_eq!(endpoint.ts_path, ["users", "userEvents"]);
    assert_eq!(endpoint.errors[0].name, "GetUserError");

    let ResponseShape::Stream(item) = endpoint.response else {
        panic!("expected stream response");
    };
    assert_eq!(item.name, "User");
}

#[test]
fn api_endpoint_macro_preserves_created_response_shape() {
    let endpoint = endpoint(__api_endpoint_descriptor_create_user());

    assert_eq!(endpoint.method, HttpMethod::Post);
    let ResponseShape::Created(response) = endpoint.response else {
        panic!("expected created response");
    };
    assert_eq!(response.name, "User");
}

#[test]
fn api_endpoint_macro_accepts_endpoint_specific_error_enum() {
    let endpoint = endpoint(__api_endpoint_descriptor_search_users());

    assert_eq!(endpoint.errors[0].name, "SearchUsersError");
    assert_eq!(endpoint.ts_path, ["users", "searchUsers"]);
}

#[test]
fn api_endpoint_macro_accepts_axum_handler_wrappers() {
    let endpoint = endpoint(__api_endpoint_descriptor_axum_wrapped_create_user());

    assert_eq!(endpoint.method, HttpMethod::Post);
    assert_eq!(endpoint.route.0, "/axum-users/{id}");
    assert_eq!(endpoint.request.path_params[0].rust_name, "id");
    assert_eq!(endpoint.request.body.expect("body").name, "User");
    let ResponseShape::Created(response) = endpoint.response else {
        panic!("expected created response");
    };
    assert_eq!(response.name, "User");
}

#[test]
fn api_endpoint_macro_accepts_axum_sse_wrapper() {
    let endpoint = endpoint(__api_endpoint_descriptor_axum_wrapped_user_events());

    assert_eq!(endpoint.method, HttpMethod::Get);
    assert_eq!(endpoint.transport, Transport::ServerSentEvents);
    let ResponseShape::Stream(item) = endpoint.response else {
        panic!("expected stream response");
    };
    assert_eq!(item.name, "User");
}

#[test]
fn api_endpoint_macro_emits_binary_download_metadata() {
    let endpoint = endpoint(__api_endpoint_descriptor_download_file());

    assert_eq!(endpoint.method, HttpMethod::Get);
    assert_eq!(endpoint.transport, Transport::BinaryDownload);
    assert_eq!(endpoint.route.0, "/files/{id}");
    let ResponseShape::Binary { content_type } = endpoint.response else {
        panic!("expected binary response");
    };
    assert_eq!(content_type, None);
}

#[test]
fn api_endpoint_macro_emits_binary_upload_metadata() {
    let endpoint = endpoint(__api_endpoint_descriptor_upload_file());

    assert_eq!(endpoint.method, HttpMethod::Post);
    assert_eq!(endpoint.transport, Transport::BinaryUpload);
    assert_eq!(
        endpoint.request.body_transport,
        RequestBodyTransport::Binary
    );
    assert_eq!(endpoint.request.body.expect("body").name, "Bytes");
    let ResponseShape::Json(response) = endpoint.response else {
        panic!("expected json response");
    };
    assert_eq!(response.name, "User");
}

#[test]
fn api_endpoint_macro_registers_reachable_types_and_errors() {
    let mut registry = typeweld_core::ContractRegistry::new();

    __api_register_endpoint_get_user(&mut registry);
    __api_register_endpoint_get_user(&mut registry);

    let types = registry.type_defs();
    let errors = registry.error_defs();

    assert_eq!(types.len(), 1);
    assert_eq!(types[0].rust_name, "User");
    assert_eq!(errors.len(), 1);
    assert_eq!(errors[0].rust_name, "GetUserError");
}

#[test]
fn api_endpoint_macro_emits_axum_handler_adapters() {
    fn accepts_handler<H, T>(_: H)
    where
        H: axum::handler::Handler<T, ()> + Clone + Send + Sync + 'static,
        T: 'static,
    {
    }

    accepts_handler(__typeweld_axum_handler_axum_wrapped_create_user);
    accepts_handler(__typeweld_axum_handler_axum_plain_user);
    accepts_handler(__typeweld_axum_handler_axum_result_user);
    accepts_handler(__typeweld_axum_handler_axum_plain_events);
    accepts_handler(__typeweld_axum_handler_axum_result_events);
    accepts_handler(__typeweld_axum_handler_axum_download_file);
    accepts_handler(__typeweld_axum_handler_axum_upload_file);
}

#[test]
fn api_router_macro_collects_mounted_endpoints() {
    let module = declarative_routes().into_route_tree().into_api_module();
    let routes = module
        .endpoints()
        .iter()
        .map(|endpoint| endpoint.route.0.as_str())
        .collect::<Vec<_>>();

    assert_eq!(module.name(), "users");
    assert_eq!(routes, ["/axum-users/plain", "/axum-users/result"]);
}

#[tokio::test]
async fn api_router_macro_mounts_generated_axum_adapters() {
    use tower::ServiceExt;

    let app = declarative_routes().into_router();
    let response = app
        .oneshot(
            axum::extract::Request::builder()
                .method(axum::http::Method::GET)
                .uri("/axum-users/plain")
                .body(axum::body::Body::empty())
                .expect("request"),
        )
        .await
        .expect("response");

    assert_eq!(response.status(), StatusCode::OK);
    let body = to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("body");
    assert_eq!(std::str::from_utf8(&body).expect("utf8"), r#"{"id":7}"#);
}

#[tokio::test]
async fn generated_axum_adapter_converts_result_success() {
    let response = __typeweld_axum_handler_axum_result_user().await;

    assert_eq!(response.status(), StatusCode::OK);
    let body = to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("body");
    assert_eq!(std::str::from_utf8(&body).expect("utf8"), r#"{"id":9}"#);
}

#[tokio::test]
async fn generated_axum_adapter_converts_result_error() {
    let response = __typeweld_axum_handler_axum_missing_user().await;

    assert_eq!(response.status(), StatusCode::NOT_FOUND);
    let body = to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("body");
    assert_eq!(
        std::str::from_utf8(&body).expect("utf8"),
        r#"{"_tag":"NotFound"}"#
    );
}

#[tokio::test]
async fn generated_axum_adapter_converts_plain_response() {
    let response = __typeweld_axum_handler_axum_plain_user().await;

    assert_eq!(response.status(), StatusCode::OK);
    let body = to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("body");
    assert_eq!(std::str::from_utf8(&body).expect("utf8"), r#"{"id":7}"#);
}
